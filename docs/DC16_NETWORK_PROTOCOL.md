...
| `0x03` *(create object)* | `0x41CF08` | `i16 obj`, `i16 x`, `i16 height`, `i16 z`, `u8 type`, `u8 owner`, `u8 heading`, `u8 life`, `u8 hp` | instantiate object `obj` (positions `value << 5`; also `MAX_OBJ = obj+1` and the allocation table); no sender found in the binary (field order corrected 11 Sep 2026) |
...
| `0x09` *(build building)* | `0x41CAA4` | `u8 slot`, `u8 level`, `u8 player` | construct or upgrade the player's city building `slot` (0..14) to `level`: the pre-allocated building object `player*15+slot` is (re)initialised instantly with full hit points; a repeat for an already built slot refunds the cost |
...
4. Pacing (`0x41E268`): while `gs->until ≥ 0` and `game_time < until` (asserted `until > game_time`): for every `tick_ms` elapsed, if `game_time + 1 == until` first run the held-frame executor (`0x41E0D8`, mode 1), which dispatches **whole held frames in order** — every command of a frame, the `UNTIL` marker included — and stops after the frame whose `UNTIL` marker cleared `gs->until`; then `game_time++` (`gs+0x94C`) and simulate one tick (`0x419978`). Executing the `UNTIL` marker asserts `until == gs->until`, sends `UNTIL(-1, until)` ("I reached `until`") and clears `gs->until`; the loop then takes the `until` of the next held frame that starts with `0x02`, if any, otherwise the client stalls until the server's next `UNTIL`. Hence the commands of a frame `UNTIL(a, u)` take effect at game time `u − 1`, before the tick that makes it `u` (server plan F38).
...
After every tick each client stores a 16-bit checksum of the game state (`0x44ABC0`: the plain 16-bit sum of the day/night fields `gs+0x534/0x538/0x53C/0x540`, the tick counter `gs+0x94C`, the highest object index `gs+0x7D40`, the missile count `gs+0x7D44`, and for every object slot 0..`gs+0x7D40` its life-state byte plus, when alive, x, z, the low 16 bits of hit points, team and type; player money is **not** included, see `DC16_BATTLE_ENGINE.md` §16) in a 256-entry history indexed by `time mod 256`.

Exact timing (read 11 Sep 2026 from `0x41E268`/`0x41E0D8`/`0x419978`, server plan F38/F39): the pacing loop increments `gs+0x94C` **before** calling `game_tick`, and `game_tick` ends with `record(gs, gs+0x94C)`, so `history[t]` is the checksum of the state *after* tick `t`. Right after the record the sender check runs (`0x419F1B`): among the game players with AI type 0 (humans, `gs+0xBBC`) and a non-negative network id (`gs+0xCA0`) the one with the **smallest network id** is chosen; if it is the local player, `0x08 (history[t], t)` with `t = gs+0x94C` is written straight to the connection (`0x421648`) as its own frame. Receivers run the handler `0x41CE74` → `check` `0x44ACF8(gs, t, value)` when the frame that carries the command is executed (i.e. at game time `until − 1` of that frame): it asserts `t ≤ gs+0x94C` (`AUGH check sync time %ld > game time %ld`) and `gs+0x94C − t < 256` (`records start at %ld`), compares `history[t % 256]` and on mismatch prints `sync error: time %ld, net %d, me %d` and calls the desync handler `0x44AC94`, which asserts and ends the game.

The relay server's fake host makes the lowest network id a player without a client, so no client sends `0x08`; the server can instead compute the checksum with its own port of the simulation and place `0x08` commands in its sync frames (server plan §18).

### 6.6 Disconnects
A read error on a server slot closes it and broadcasts `DISCONNECT(slot)`. In the lobby the slot becomes type 3 / status 0. In-game the player's control changes to AI (`+0xBBC = 3`), its money is normalised and a system chat message "*name* lost, AI taking over" is generated locally. A client whose own read fails leaves the game with "Connection Lost"/"SERVER LOST!?!". The "Connection Lost" path is in the client step `0x40AAFC` (CW `0x40AB5C`): the battle loop `0x40115C` calls the client step on every UI pass while `gs+0x471C8` is 0, the step calls the receive loop `0x41E648` before anything else (`0x40ABD7`), and a negative return sets `gs+0x471C8 = 1` and shows error text 2 through `0x410400(net, 2, 8)` (`0x40ABDC..0x40ABF7`). So that box means the client was alive and reading when the server closed the socket - unlike a crash (Windows Error Reporting keeps the socket open, the relay sees only missing echoes) or a sync assert (`error.log` line + message box, no "Connection Lost"). Relay side, the box follows every eviction of a live client, e.g. `no echo for frame N` after `ECHO_TIMEOUT_MS` (server plan F66).
...
* `ENGEXP16.EXE` (Council Wars) is the same code base with slightly different addresses (+0x60 from `0x4063E4` on). Since 25 Sep 2026 verified by a relocation-aware byte comparison of the untouched exes: the network, lobby, lockstep and checksum code is identical to Classic's, so every message format here holds for it too; the two builds differed only in simulation data (Gray commander deploy animation, sticky menu mode), fixed in the patched "Dark Colony Ultimate.exe" (`RELAY_SERVER_PLAN.md` F65, `DC16_DISPLAY_AND_RESOLUTION.md` §10.39). Not yet seen on the wire.
* Command `0x0C` sets a per-player upgrade level: `which` 0 = weapon slot, 1 = armour multiplier, for unit type `slot` (`object_types[slot]+0x30/+0x38` in `dc16.exe`), cost `1000 × value`. Order codes are indices into the order-handler table `0x4893D4` (`0x0E` attack target, `0x12` fire at ground / targeted special, `0x0D` deploy); the unit state machine, the tables `0x50FDFC`/`0x50FE04` (object-type fields `+0x104`/`+0x10C`) and the whole combat simulation are documented in `Dark-Colony/docs/DC16_BATTLE_ENGINE.md`. The trailing bytes of `0x03` are still untraced.
