...
| | left column | right column | stock label (untouched exe) |
|---|---|---|---|
| row 1 | **ACADEMY** (1) | MULTI PLAYER WAR (3) | TRAINING |
| row 2 | **COUNCIL WARS** (0) | — | NEW CAMPAIGN |
| row 3 | **LOAD CW GAME** (2) | ENCYCLOPEDIA (5) | LOAD GAME |
| row 4 | **OZI MISSIONS** (16) | — | PLAY INTRO |
| row 5 | **LOAD OZI GAME** (4) | QUIT (12) | SINGLE PLAYER WAR |
...
second column's rows 3 and 5). It is **five** rows, so `build_ozi_overlay.menu_layout` is a layout
step again, not two renames: the ids stay with their handlers (the exe patch rewires 16 and 4, §10.13),
only positions and labels move, each gadget follows its button, and the `banim` pairs are untouched.
The block is anchored on the **bottom** row of the grid it is given — the row the 640×480 artwork sits
3 px below — so the fifth row is won at the top and applying the step twice changes nothing (the
pitch comes from the script, the bottom row from `max(y)`, both identical before and after).

**Why the grid is 16 px lower, and why the credits box moves in patched builds** (maintainer, same day: "for unpatched engexp16.exe put credits a bit higher and buttons a bit lower so they don't overlap each other"). The scrolling `credits.txt` TTY (280×100, `main.c bintro` `0x00404DC8`) is the one element of this screen that the **exe** positions, from two imm32 — Classic (178, 200), Council Wars (178, **230**), because the expansion's four-row column started 26 px lower (§10.7, §10.11). Classic's grid starts at 314, so the 230 box (rows 230..329) would cover the top 16 px of the first row. In the **untouched** exe the box cannot move — it is an immediate, not data — so there the buttons take the whole step: first row **330**, one row below the box, bottom row ending at 432, and the backdrop's bottom artwork starts at row **436** (`exp/intrface/intrg.gif`, re-measured: the button band x 138..496 is black to row 435), which leaves 3 px. That is the whole budget, and it is also why the five-row patched menu cannot simply grow downwards: it grows upwards instead, into the space the box has to vacate. So in every patched build the box goes to **8 px above the first of the five rows** — stock-equivalent y = 304 − 100 − 8 = **196** — which is `credits_y(196, 296)` in `patch_resolution.py` (296 = the cluster centre of the stock script, rows 159..433) with `paint_intro.CREDITS_STOCK_Y` following it and the stock immediate kept as `CREDITS_EXE_IMM_Y`. At 640×480 there is no `resolution` fix, so the **`ozi` fix writes the same 196 itself** (new 640×480-only site in `patch_ozi_menu.py`: `mov ebx,230` → `mov ebx,196` at `0x00404E99`, file `0x4299`, that mode's `ozi` edit count 15 → 17 counting the `.reloc` insert). Measured on the generated scripts, every mode lands identically: **rows at pitch 26 with the bottom row unmoved, the box 8 px above the first row and the `DCUT` title 4 px above the box** (640×480 304..432 / box 196..295; 1024×768 502..630 / 394..493; 1280×720 472..600 / 364..463; 1280×800 521..649 / 413..512; 1280×1024 659..787 / 551..650; 3840×1080 694..822 / 586..685), and the painted HD backdrops are black under the button band well below every bottom row (724 / 676 / 756 / 980). Published Council Wars builds after this: 1024×768 SHA-256 `a4107c2f…`, 1280×800 `7c1c9608…`, 640×480 `a7223536…`; Classic is untouched (`a71d038b…`).

**What the toolchain does now.** Every derived script is letterboxed/relayouted from the one stock file, so the grid propagates by itself and the OZI mode is a small layout step on top of it: `build_ozi_overlay.menu_layout` / `menu_script` and the patcher's `Edit-OziMenu` place the eight buttons (and their gadgets) in the five-row order above and rewrite five `textmsg` lines — `COUNCIL WARS`, `ACADEMY`, `LOAD CW GAME`, `LOAD OZI GAME`, `OZI MISSIONS`; `MULTI PLAYER WAR`, `ENCYCLOPEDIA` and `QUIT` keep their stock texts. Both refuse a script that is not the complete 2×4 grid (all eight `pushb`, all eight `gadget`, `banim` with 8 pairs) and name what is missing, and both are idempotent. `Edit-OziMenu640` — the two-column special case of 21 Sep 2026 (§10.27), needed only because five single-column rows did not fit between the credits box and the bottom artwork — is gone: with the box out of the way the same two-column block fits at every size, which is what that plan was reaching for.
...
**Checks made.** Regenerated `exp/intrf_hd/bintroe` for every mode through the patcher's own `Edit-OziMenu (Set-BackgroundHd (Edit-IntroScript …))` chain (`hd_sets/` fixtures for the four shipped sizes, the game folder's live 1280×800 pair and the `ozi_ns/intrf_hd` copy) and checked per mode that every greyed id is defined, that the five rows sit at pitch 26 with the bottom row where the 2×4 grid had it, that the credits box clears the first row by 8 px and the title by 4, and that the bottom row clears the backdrop artwork. `build_ozi_overlay.py` reports `0 edits` against the result and refuses a script without the grid; `menu_script` applied twice is a no-op. The patcher rebuilt all three Council Wars modes from the stock exe and matched its own references (1280×800 `7c1c9608…`, 1024×768 `a4107c2f…`, 640×480 `a7223536…`, whose `ozi` run also wrote `exp\intrface\bintoze` and `ozi_ns\intrface\bintoze` with the five-row layout), and an earlier run of the same rehearsal had rebuilt the maintainer's previous 1280×800 build byte for byte, which is what made the one-byte credits differences easy to confirm. The game folder's build was refreshed to `7c1c9608…`. The two campaign scene lists are the only files where the rehearsal copy differs — it was made without `AVI\`, and `HSCENE`/`GSCENE` keep the Classic ending names only when the `DC*.AVI` files are there (§10.18).
