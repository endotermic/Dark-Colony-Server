...
| `ai.js` | `ai.c` (19 Sep 2026): `ai_turn` `0x41AE38` after `record(t)` (literal tick 4, then one slot per call), `ai_think` `0x41AD30` (one `rand()` per personality pair, `w > r·(cum+w)/32767`), the personality table, the triggers `ai`/`aimsg`; types 1/2 are stubs (campaign, CRT `rand()`) | `aiTurn(G)`, `aiThink(G, p)`, `exactContext(G, p)`, `triggerAi`, `triggerAiMsg` |
| `krusty.js` | `krusty.c`, `krusty_general.c`, `krusty_attack.c`, `krusty_defend.c`, `krusty_scout.c`, `krusty_army.c` (19 Sep 2026; `docs/DC16_AI.md` §5-§15 and the corrections of §23): the 0x6C40-byte state with the original offsets, zone table, influence map, census, goals, the four tasks, the mover. Driven through a `ctx` `{ G, p, kai, rand, emit, say, assert, exact }` so that the same code runs inside `game_tick` (exact) and as a server bot (`src/krustybot.js`, private RNG, commands into the sync frame) | `krustyAlloc(G, p)`, `krustyThink(ctx)`, `krustyAimsg(ctx, words)`, `summarize(G, kai)` |
